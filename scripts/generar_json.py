...
RESPONSABILIDAD: Convertir los datos procesados (CSV) en archivos JSON
                 que la página web pueda consumir directamente.

¿POR QUÉ JSON Y NO CSV?
  - Los navegadores web NO pueden leer archivos CSV directamente
  - JSON es el formato nativo de JavaScript
  - Es más fácil de usar en HTML/JS
  - Permite estructuras de datos más complejas

Archivos que genera:
  web/data/ranking_elo.json      → Ranking ELO de las 48 selecciones
  web/data/estadisticas.json     → Estadísticas históricas
  web/data/predicciones.json     → Probabilidades por partido (Poisson)
  web/data/simulacion.json       → Probabilidades de ser campeón
  web/data/metadata.json         → Fecha de última actualización
"""
...
def csv_a_json(archivo_csv, archivo_json, nombre):
    ...
    # Guardar como JSON con formato legible (indent=2 → sangría de 2 espacios)
...
def generar_json():
    """Función principal: convierte todos los CSV a JSON."""
    ...
    csv_a_json(ARCHIVOS_ENTRADA['estadisticas'], ARCHIVOS_SALIDA['estadisticas'], "Estadísticas")
    csv_a_json(ARCHIVOS_ENTRADA['predicciones'], ARCHIVOS_SALIDA['predicciones'], "Predicciones Poisson")
    csv_a_json(ARCHIVOS_ENTRADA['simulacion'], ARCHIVOS_SALIDA['simulacion'], "Simulación Mundial")
    
    # Generar metadata con la fecha y hora de la última actualización
    metadata = {
        "ultima_actualizacion": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "ultima_actualizacion_legible": datetime.now().strftime("%d de %B de %Y, %H:%M"),
        "version": "1.0.0",
        "descripcion": "Fuchibol26 - Sistema de predicción del Mundial 2026"
    }
    
    with open(ARCHIVOS_SALIDA['metadata'], 'w', encoding='utf-8') as f:
        json.dump(metadata, f, ensure_ascii=False, indent=2)
    
    print(f"  ✓ Metadata: fecha de actualización guardada")
    
    print(f"\n✅ Archivos JSON generados en: {RUTA_WEB_DATA}")
    print("   La página web ya puede leer los datos actualizados")
